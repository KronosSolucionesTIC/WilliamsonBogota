...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar($t_usuario,$eli_codigo,"cod_usu");
	if ($error >=1)
...
$sql="SELECT cod_usu,nom_usu , cc_usu, nom_car FROM usuario INNER JOIN cargo ON cod_car=car_usu $where  ORDER BY nom_usu ";
                  ...
                  <table width="624" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_usuario.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						$estilo="ctablablanc";
						$estilo="ctablagris";
						//$sql="SELECT cod_usu,nom_usu , cc_usu,des_car FROM usuario $where $paginar";
						//echo $sql;
						...
						while($db->next_row()) {
							...
							$nombre_usuario = letra_capital($db->nom_usu);
							$cargo = letra_capital($db->nom_car);
							$cc=convertir_cedula($db->cc_usu);
							...
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.gif' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_usu.submit()'/></td>";
							else 
