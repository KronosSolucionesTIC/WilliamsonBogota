...
$db= new  Database();
$sql="SELECT cod_mod, nom_rol, nom_mod, cod_int_per, nom_int, rut_int, cod_usu_per, con_per, edi_per, ins_per, eli_per, concat( CONCAT( CONCAT( CONCAT( CONCAT( CONCAT( CONCAT( CONCAT( CONCAT( rut_int, '?consulta=' ) , cod_usu_per ) , '&editar=' ) , edi_per ) , '&insertar=' ) , ins_per ) , '&eliminar=' ) , eli_per ) , '&pagina=1' ) AS rutas, cod_per
FROM permisos
INNER JOIN interfaz ON permisos.cod_int_per = interfaz.cod_int
INNER JOIN modulos ON interfaz.cod_mod_int = modulos.cod_mod
INNER JOIN rol ON cod_rol = cod_usu_per
INNER JOIN usuario ON cod_rol = cod_rol_usu
WHERE usuario.cod_usu=$global[2]  AND con_per>0 ORDER BY modulos.cod_mod ,cod_per asc"; 
#echo $sql;
#exit;
$db->query($sql); 
$aux=1;
...
var tmenuItems = [
    ["Menu del Sistema","", "menus/data.files/xpicon1_s.gif", "", "", "Menu del Sistema", "", "0", "", ],
        ["|Inicio","presentacion.php", "menus/data.files/icon2_s.gif", "menus/data.files/icon2_so.gif", "", "Inicio", "", "", "", ],     
			<? while($db->next_row()) 
			{			
