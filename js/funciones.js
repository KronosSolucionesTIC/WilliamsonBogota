...
function validaInt(cadena, obj, e){
opc = false;
...
function validaInt_evento(a,obj){
	...
	if( event.keyCode ==13  )  
	{
		if(a.value.length < 1) 
		{
			alert('Referncia No Encontrada')
			document.getElementById("codigo_fry").value='';
			document.getElementById("codigo_fry").focus();
			document.getElementById("producto_combo")[0].selected = true;
			vandera=1;
			return false;
		}
...
function confirmar_eli(codigo1,codigo2)
	{  
		if(confirm("¿Esta seguro de Eliminar el Registro?.")) 
		{
			document.getElementById('eliminacion').value=1;
			document.getElementById('eli_codigo').value=codigo1+'|'+codigo2;
			document.forma.submit();		
		} 
	} 
...
function abrir_popup(ruta,codigo,tamano){
var ancho=0;
var alto=0;
