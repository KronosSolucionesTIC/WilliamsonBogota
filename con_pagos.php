...
if($eliminacion==1) {//confirmacion de eliminacio 
	$error=eliminar("factura",$eli_codigo,"cod_fac");
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	INNER JOIN cliente ON (cliente.cod_cli = factura.cliente)
 	...
 	$where AND tipo_factura = 1 order by cod_fac  desc";
...
<form id="forma_total" name="forma_total" method="post" action="con_pagos.php">
                  <table width="790" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td width="790" bgcolor="#E9E9E9"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td width="8" height="33"> </td>
                          <td width="17"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/page.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='con_pagos2.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" class="ctablaform"><select name="campos" class="textfieldlista" id="campos" >
                            <option value="0">Seleccion</option>
                               <option value="cod_fac">Factura</option>
							<option value="fech_factura">Fecha factura</option>
                    ...
                    <tr>
                      <td><table width="798" border="0"  cellpadding="0">
                        <tr>
						  <td width="124"  class="ctablasup">N&deg; FACTURA </td>
						  <td width="186"  class="ctablasup">CLIENTE</td>
						  <td width="168"  class="ctablasup">FECHA FACTURA </td>
						   <td  class="ctablasup"  width="145">OPCIONES</td>
                        </tr>
						<?php 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_pagos.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row())
						{
							if ($aux==0)
							{ 
							$estilo="ctablablanc"; $aux=1; $cambio_celda=$celda_blanca; }else { $estilo="ctablagris";  
							$cambio_celda=$celda_gris; $aux=0;
							}
							
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_fac'  
							action='man_pagos.php'>  "; 
							   
							echo "<td >$db->cod_fac</td>";
							echo "<td >$db->nom1_cli $db->apel1_cli</td>";
							echo "<td >$db->fech_factura</td>";
                           ...
                           echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\"  onclick=\"abrir($db->cod_fac,'$db->tipo_factura')\" /></td>";
						    echo "  </tr> </table>  </td>  ";
                    ...
                    <tr>
                      <td><img src="imagenes/lineasup2.gif" width="795" height="4" /></td>
                    </tr>
                    <tr>
...
<form name="forma" method="post" action="con_pagos.php">
