...
echo  " <script language='type/javascript'>alert();</script>";
...
<?php 
//echo "sitio_id es  ".$sitio_id;
$imglib = $HTTP_POST_VARS['lib'];
//echo "lib  es  ".print_r($imglib);
...
function uploadImg($img) {
  ...
  if ($_FILES[$img]['size']>0) {
    ...
    // get file extension
    $ext = strtolower(substr(strrchr($data['name'],'.'), 1));//extension de la imagen
    //spaw_valid_imgs es el arreglo de imagenes permitidas
    if (in_array($ext,$spaw_valid_imgs)) {
//        $_root="/home/kaomecom/public_html/clientes/alinova/";
       $_root = $_SERVER['DOCUMENT_ROOT'];
	   $_root .= "/net-kargo/";
